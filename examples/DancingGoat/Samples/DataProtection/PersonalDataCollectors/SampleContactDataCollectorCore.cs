    ...
    internal class SampleContactDataCollectorCore
    {
        ...
        private void WriteConsents(ICollection<int> contactIDs)
        {
            ...
            foreach (var row in consentsData.Tables[0].AsEnumerable())
            {
                ...
                if (!consents.TryGetValue(consentAgreementInfo.ConsentAgreementConsentID, out ConsentInfo consentInfo))
                {
        ...
        {
            if (!consentRevocations.TryGetValue(consentId, out List<ConsentAgreementInfo> revocationsOfSameConsent))
            {
        ...
        {
            if (!consentContentAgreements.TryGetValue(consentHash, out List<ConsentAgreementInfo> agreementsOfSameConsent))
            {
            ...
            Dictionary<string, List<ConsentAgreementInfo>> consentContentAgreements, Dictionary<int, List<ConsentAgreementInfo>> consentRevocations)
        {
            foreach (var agreementsOfSameConsentContent in consentContentAgreements.Values)
            {
                ...
                consentContentArchives.TryGetValue(consentAgreement.ConsentAgreementConsentHash, out ConsentArchiveInfo consentArchiveInfo);

                consentRevocations.TryGetValue(consentAgreement.ConsentAgreementConsentID, out List<ConsentAgreementInfo> revocationsOfSameConsent);
        ...
        private void WriteContactAccounts(ICollection<int> contactIDs)
        {
            ...
            {
                countryInfos.TryGetValue(accountInfo.AccountCountryID, out CountryInfo countryInfo);
                stateInfos.TryGetValue(accountInfo.AccountStateID, out StateInfo stateInfo);
